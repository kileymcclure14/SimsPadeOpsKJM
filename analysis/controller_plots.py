...
# **STRICTLY USE LAST 10,000 TIMESTEPS** ← FIXED VALUE
MAX_TIMESTEPS = 10000
...
def get_selected_tidx(path, runid, max_timesteps=MAX_TIMESTEPS):
    """
    Get the last max_timesteps available tidx values.
    ...
    runid : int
        Run ID
    max_timesteps : int
        Number of most recent timesteps to use
    
    Returns
    -------
    list
        Selected tidx values (last max_timesteps available)
    """
    all_tidxs = get_all_tidx(path, runid)
    
    # Always use the LAST max_timesteps
    selected = all_tidxs[-max_timesteps:]
...
for idx, cfg in enumerate(SIM_CONFIGS, 1):
    ...
    try:
        # Get last MAX_TIMESTEPS available tidx values for this simulation
    ...
    try:
        ...
        # Load each selected budget timestep
        ...
        # Time-average the TI values (spatial average)
        ...
        print(f"    - TI at x={X_EVAL}: {np.nanmean(np.array(ti_temporal_dict['ti_at_x4'])):.3f}%")
    ...
    try:
        ...
        if ti_file and (cfg["CTP"], cfg["blockage"]) not in ti_data_all:
            ...
            valid_mask = ~(np.isnan(ti_fact_data) | np.isinf(ti_fact_data) | np.isnan(tidx_data) | np.isinf(tidx_data))
            tidx_data = tidx_data[valid_mask]
            ti_fact_data = ti_fact_data[valid_mask]
            
            # Calculate mean of TI_fact
            mean_ti_fact = float(np.mean(ti_fact_data)) if len(ti_fact_data) > 0 else np.nan
...
for ctp in ctps:
    for blockage in blockages:
        for ti_level in ti_levels:
            ...
            fig.suptitle(f"CTP={ctp}, {blockage} Blocked, {ti_level} TI\n(Time-Averaged over {len(matching_result['selected_tidxs'])} budget timesteps)", 
                         fontsize=14, fontweight='bold')
            
            # ========== ROW 1: SPATIAL PROFILES ==========
            
            # --- Subplot 1: TI_factor vs TIDX with Mean Line ---
            ax1 = axes[0, 0]
            if has_ti_factor:
            ...
            # --- Subplot 2: Mean TI Profile (Spatial) - SINGLE LINE ---
            ax2 = axes[0, 1]
            ...
            # --- Subplot 4: TI vs Time (at x=X_EVAL)
            ax4 = axes[1, 0]
            ...
            if temporal_key in ti_temporal_data:
                ...
                ax4.plot(temporal["tidx"], temporal["ti_at_x4"], 'o-', 
                        color=matching_result["color"], linewidth=2.5, markersize=7, alpha=0.7)
