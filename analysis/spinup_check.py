...
tids = range(0, 13620, 10)
u, v, w = [], [], []

for tid in tids:
    data_f = sim.slice(field_terms=["u", "v", "w"], xlim=6.25, ylim=6.25, zlim=6.25, tidx=tid)
...
print(f"u.shape = {u.shape}")  # Sanity check

# PLOT 1: TKE evolution with stationarity check
ke = 0.5 * np.mean(u**2 + v**2 + w**2, axis=(1, 2, 3))
tke_final = ke[-10:].mean()
tke_std = ke[-10:].std()

plt.figure(figsize=(10, 6))
plt.plot(tids, ke, 'o-', label="TKE", linewidth=2)
...
plt.savefig("./TKE_stationarity_UNB.png", dpi=300)
plt.close()
...
times_to_plot = [len(all_Ek)//8, len(all_Ek)//4, len(all_Ek)//2, -1]
colors = ['red', 'orange', 'green', 'blue']
for i, nt in enumerate(times_to_plot):
    mask_t = (k_shell > 0) & (cumulative_means[nt] > 0)
    plt.loglog(k_shell[mask_t], cumulative_means[nt][mask_t], 
...
plt.savefig('spectrum_convergence_UNB.png', dpi=300)
plt.close()

# PLOT 3: Final spectra + Kolmogorov check
i_ref = len(k_plot) // 4
k_ref = k_plot[i_ref]
...
plt.tight_layout()
plt.savefig('fully_developed_spectra_UNB.png', dpi=300)
plt.close()