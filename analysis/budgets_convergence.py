...
# Get Velocities
tids = range(5000, 12792, 1000)
all_t = sim.unique_times()

valid_tids = [(i, tid) for i, tid in enumerate(tids) if i < len(all_t)]

for i, tid in valid_tids:
    data_ubar = sim.slice(budget_terms=["ubar"], ylim=0.99, zlim=0.99, tidx=tid)
    data_vbar = sim.slice(budget_terms=["vbar"], ylim=0.99, zlim=0.99, tidx=tid)
    data_wbar = sim.slice(budget_terms=["wbar"], ylim=0.99, zlim=0.99, tidx=tid)
...
axes[0].set(xlabel="x", ylabel="ubar", title="Ubar Convergence (20% Blockage)")
axes[1].set(xlabel="x", ylabel="vbar", title="Vbar Convergence (20% Blockage)")
axes[2].set(xlabel="x", ylabel="wbar", title="Wbar Convergence (20% Blockage)")
...
plt.suptitle("Mean Velocity Profiles Convergence in 20% Blocked Domain", fontsize=16)
plt.savefig("20pct_convergence.png", dpi=300, bbox_inches="tight")
plt.close()